  ...
  async execute(context) {
    ...
    const menu = new Menu(
      ...
      radios
        ...
        .map(r => `\`${r.id}\`: ${r.emote} [${r.name}](${r.website})${!r.broken ? ` - ${r.language} (${r.country}) - ${r.type.map(t => context.__(`radio.types.${t}`)).join(', ')}` : context.__('radio.broken')}`),
    );
...
class InfoSubcommand extends Command {
  ...
  async execute(context) {
    const currentBroadcast = this.client.radio.broadcasts.find();

...
class WeatherCommand extends Command {
  ...
  async execute(context) {
    ...
    const currently = [
      ...
      `${this.dot} ${context.__('weather.embed.wind')}: ${context.__('weather.wind', {
        direction: context.__(`weather.wind.${this.getDirection(weatherData.currently.windBearing)}`),
        speedKph: Math.floor(weatherData.currently.windSpeed),
        speedMph: Math.floor(weatherData.currently.windSpeed / 1.609),
        gustKph: Math.floor(weatherData.currently.windGust),
        gustMph: Math.floor(weatherData.currently.windGust / 1.609),
      })}`,
    ...
    for (let i = 0; i < weatherData.daily.data.length; i += 1) {
      ...
      entries.push([
        ...
        `${this.dot} ${context.__('weather.embed.wind')}: ${context.__('weather.wind', {
          direction: context.__(`weather.wind.${this.getDirection(item.windBearing)}`),
          speedKph: Math.floor(item.windSpeed),
          speedMph: Math.floor(item.windSpeed / 1.609),
          gustKph: Math.floor(item.windGust),
          gustMph: Math.floor(item.windGust / 1.609),
        })}`,

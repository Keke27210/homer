  ...
  async execute(context) {
    ...
    for (let i = 0; i < weatherData.daily.data.length; i += 1) {
      ...
      pages.push([
        ...
        `${this.dot} ${context.__('weather.embed.precip')}: ${item.precipAccumulation < 0.5 ? `**${context.__('global.none')}**` : `**${context.__(`weather.precip.${item.precipType}`)}** - **${Math.floor(item.precipAccumulation * 100) / 1000}**mm (**${Math.floor((item.precipAccumulation / 25.4) * 100) / 1000}**in)`}`,

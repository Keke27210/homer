...
class BaninfoCommand extends Command {
  ...
  async execute(context) {
    ...
    if (search) {
      ...
      return context.replyError(context.__('finderUtil.findUsers.noQuery'));
    }
    ...
    const embed = new RichEmbed()
      .setDescription([
        ...
        `${this.dot} ${context.__('baninfo.embed.executor')}: **${logEntry.executor.username}**#${logEntry.executor.discriminator} (ID:${logEntry.executor.id})`,
        `${this.dot} ${context.__('baninfo.embed.reason')}: ${logEntry.reason ? `**${logEntry.reason}**` : context.__('global.none')}`,
        `${this.dot} ${context.__('baninfo.embed.date')}: **${context.formatDate(logEntry.createdTimestamp)}**`,
      ].join('\n'));

...
class AboutCommand extends Command {
  ...
  async main(message) {
    const owners = [];
    for (let i = 0; i < this.client.constants.owners.length; i += 1) {
      const user = await this.client.users.fetch(this.client.constants.owners[i])
        .catch(() => null);
    ...
    const description = [
      ...
      `${message.dot} ${message._('about.memory')}: **${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)}**MB`,
